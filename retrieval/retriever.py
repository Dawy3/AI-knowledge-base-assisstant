...
class AdvancedRetriever:
    ...
    def retrieve(
        ...
        if self.enable_routing:
            ...
            if classification["type"] == QueryType.CLARIFICATION: # Ambiguous query
                return {
    ...
    def _merge_results(self, results: List[List[Dict]]) -> List[Dict]:
        ...
        Args:
            results: List of lists, where each inner list contains result dicts from one query variant
        ...
        # Iterate over each query variant's results
        for query_results in results:
            # Iterate over each result in this query variant
